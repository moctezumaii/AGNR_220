...
### Select the systolic blood pressure variable

The column is named `systolicBP`. The `$` symbol selects that column from the dataframe.

```{r}
systolic <- blood$systolicBP
```

R is case-sensitive. `Systolic` and `systolic` are different names. Use the exact column name shown by `names(blood)`.
...
quantile(systolic, probs = 0.25)

quantile(systolic, probs = 0.75)
